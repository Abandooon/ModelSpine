...
from offline import demo
from fixtures import load_fixture
from modelspine_assurance import TOOL, VERSION, check
from modelspine_generation import construct, plan
...
from modelspine_protocols import (
    ...
    Obligation, SetDependencies, SetProperty, ref,
)
...
class KernelIntegrationTests(unittest.TestCase):
    ...
    def test_bypassed_construction_is_rejected_by_terminal_check(self):
        illegal = self.proposal(SetProperty('set_property', 'approval-policy', 'threshold_minor', -1))
        report = check(self.kernel.preview(illegal).candidate, self.plan)
